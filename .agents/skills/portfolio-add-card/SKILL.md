...
> drift apart. See AGENTS.md → "Bilingual structure" for the full rules.
...
before `</head>` (relative path — see `AGENTS.md` → Analytics for why not
absolute). Grep the target file; add the tag if it's missing.

## 1 · Inspect the target page

Use the running preview server (port 60270, server ID in `.Codex/launch.json`)
